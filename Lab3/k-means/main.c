...
void kmeans(int *assignment, int K, int max_iter, int n_samples, int m_features, double *data){
	...
	for(int n = 0; n < max_iter; n++){
		...
		for(int sample = 0; sample < n_samples; sample++){
			...
			for(int d = 0; d < m_features; d++){
				centroid_sums[assignment[sample]*m_features + d] += data[sample*m_features + d];
			}
		...
		for(int s = 0; s < K; s++){
			for(int i = 0; i < m_features; i++){
				centroids[s * m_features + i] = centroid_sums[s * m_features + i]/centroid_datapoints[s];
			}
		}
